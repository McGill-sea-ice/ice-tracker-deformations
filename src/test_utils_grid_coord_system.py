...
for idx in range(len(sLon)):
    ...
    j, i = grid_cs.find_nearestGridTracerPt(grid['X_aeqd'],grid['Y_aeqd'], (xs[idx],ys[idx]))
    dist=haversine((sLat[idx], sLon[idx]),(grid['LAT'][j,i], grid['LON'][j, i]))
...
for n in range(len(tri.simplices)):
    ...
    center_x, center_y = grid_cs.find_aeqdTriCenter(x1_aeqd, x2_aeqd, x3_aeqd, y1_aeqd, y2_aeqd, y3_aeqd)
    ...
    j, i = grid_cs.find_nearestGridTracerPt(grid['X_aeqd'],grid['Y_aeqd'], (center_x, center_y))
    center_gridlons.append( grid['LON'][j, i])
    center_gridlats.append( grid['LAT'][j, i])
...
CS = grid_cs.define_gridCS(grid['fLAT'], grid['fLON'], (200, 200))

for i in [-1, 0]:
    for j in [-1, 0]:
        xgridCS, ygridCS = grid_cs.get_xy_gridCS( CS, (grid['fLAT'][200+j, 200+i], grid['fLON'][200+j, 200+i]) ) 
...
for i in [-1, 0, 1]: 
    for j in [-1, 0, 1]:
        xgridCS, ygridCS = grid_cs.get_xy_gridCS( CS, (grid['LAT'][200+j, 200+i], grid['LON'][200+j, 200+i]) ) 
