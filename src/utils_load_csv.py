...
def load_raw_csv( path_raw_csv ):
    ...
    Loads data from a raw .csv or .dat file. Returns starting and ending point
    positions in latitudes and longitudes: (sLat, sLon, eLat, eLon). 
    
    An error is raised if the file is not a .dat nor a .csv file, or if the file 
    ...
    # Determine if the raw file is a .dat or a .csv file
    file_type = path_raw_csv[-3:len(path_raw_csv)]

    # Set the file data seperator in function of the file type
    if file_type == 'csv':
        sep = ','
    ...
    # If the file is not a .csv nor a .dat file, raise an error
    else:
        raise FileFormatError(os.basename(path_raw_csv) + \
                        ' is not a .dat nor a .csv file.')

    # Create a data frame
    df = pd.read_csv(path_raw_csv, sep=sep, engine='python')
    ...
    if ( len(sLon) < 3 ):
        ...
        # Retrieve the raw csv filename and raise an error
