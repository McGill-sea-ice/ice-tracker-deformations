...
def compute_deformations():
    ...
    for converted_csv_path, calculations_csv_path in zip(config.csv_paths['converted'], config.csv_paths['calculations']):
        ...
        if os.path.exists(calculations_csv_path) and not config.args.overwrite:
            continue
        ...
        _, sX1,sX2, sX3, sY1, sY2, sY3, eX1, eX2, eX3, eY1, eY2, eY3, _, _ = load_csv.load_converted_csv(converted_csv_path)
        ...
        dt = dt_raw_csv.dT( dt_raw_csv.dataDatetimes(calculations_csv_path) )
        ...
        os.makedirs(os.path.dirname(calculations_csv_path), exist_ok=True)
